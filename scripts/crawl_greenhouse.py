#!/usr/bin/env python3
"""Greenhouse discovery helper.

Tries search-driven discovery for direct Greenhouse posting URLs and logs new URLs.
"""
...
def extract_greenhouse_links(html: str) -> list[str]:
    links = []
...
def main() -> None:
    ...
    queries = [f'site:job-boards.greenhouse.io "{title}"' for title in titles]

    summary = {"runAt": now_iso(), "queries": len(queries), "newUrls": 0, "errors": []}
    for query in queries:
        try:
            html = search_html(query)
            for url in extract_greenhouse_links(html):
                if url in seen:
                    continue
                seen.add(url)
                summary["newUrls"] += 1
                append_jsonl(discovered, {"discoveredAt": now_iso(), "source": "greenhouse", "query": query, "url": url})
            time.sleep(1.1)
